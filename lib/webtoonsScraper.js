...
class WebtoonsScraper {
  ...
  async episodeImages(url) {
    ...
    // Webtoons.com populates real panel images via client-side JS after
    // load — a plain HTTP fetch only ever sees transparent placeholders, so
    // `images` will almost always come back empty here. This is expected,
    // not a bug; callers should show a "read on webtoons.com" link instead
    // of an in-app reader for this source.
    return images;
  }
}
