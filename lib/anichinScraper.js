...
class AnichinScraper {
  ...
  async detail(slug) {
    const json = await apiGet(`/api/detail?slug=${encodeURIComponent(slug)}&type=donghua`);
    const d = unwrapData(json);
  ...
  async episode(slug) {
    const json = await apiGet(`/api/episode?slug=${encodeURIComponent(slug)}&type=donghua`);
    const d = unwrapData(json);
